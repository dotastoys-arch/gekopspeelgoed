...
interface Customer { name: string; email: string; city: string | null; }
interface Order {
  id: number;
  category: Category;
...
export default function BestellingenPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-gray-400 text-sm mb-6">Beheer en verwerk klantbestellingen</p>
        ...
        {["pending", "packed", "shipped"].map((s) => {
          ...
          return (
            <div key={s} className="bg-white rounded-xl px-5 py-3 shadow-sm flex items-center gap-3">
              <span className="font-black text-2xl" style={{ color: st.color }}>{count}</span>
              <span className="text-sm text-gray-500">{st.label}</span>
            </div>
          );
        })}
        <div className="bg-white rounded-xl px-5 py-3 shadow-sm flex items-center gap-3 ml-auto">
          <span className="font-black text-2xl text-green-600">{euroFmt(totalProfit)}</span>
          <span className="text-sm text-gray-500">Totale winst</span>
        </div>
      </div>
        ...
        {filtered.map((order) => {
          ...
          return (
            <div key={order.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
              ...
              >
                <span className="text-xl">{CATEGORY_EMOJI[order.category]}</span>
                ...
                <div className="text-right">
                  <div className="font-bold text-green-600">{euroFmt(order.profit)}</div>
                  <div className="text-xs text-gray-400">winst</div>
                </div>
              ...
              {isOpen && (
                <div className="border-t border-gray-50 px-5 py-4 bg-gray-50/30">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
                    <div>
                      <div className="text-xs text-gray-400 mb-1">Klant</div>
                      <div className="font-semibold">{order.customer.name}</div>
                      <div className="text-gray-500">{order.customer.email}</div>
                      {order.customer.city && <div className="text-gray-400">{order.customer.city}</div>}
                    </div>
                    <div>
                      <div className="text-xs text-gray-400 mb-1">Financieel</div>
                    ...
                    <div>
                      <div className="text-xs text-gray-400 mb-2">Status wijzigen</div>
                      <select
                        value={order.status}
                      ...
                      {order.status === "pending" && (
                        <p className="text-xs text-gray-400 mt-1">
                          Voorraad wordt afgetrokken bij "Ingepakt"
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    {order.orderItems.map((item, i) => (
                      <div key={i} className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-gray-400">{euroFmt(item.purchasePriceExcl)} inkoop</span>
                      </div>
                    ))}
                  </div>
                </div>
